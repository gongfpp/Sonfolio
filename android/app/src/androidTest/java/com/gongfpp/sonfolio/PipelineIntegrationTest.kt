...
class PipelineIntegrationTest {
    ...
    @Test fun stableConversationsMarkersFilteringAndSearch() = runBlocking {
        ...
        try {
            ...
            assertEquals(2, repository.observeSearch("", "仅标记").first().hits.size)
            assertTrue(repository.observeSearch("%", "全部").first().hits.isEmpty())
            assertTrue(repository.observeSearch("_", "全部").first().hits.isEmpty())
            assertTrue(repository.observeSearch("", "全部").first().hits.isEmpty())
