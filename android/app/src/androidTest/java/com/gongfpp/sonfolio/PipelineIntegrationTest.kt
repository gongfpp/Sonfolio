...
class PipelineIntegrationTest {
    ...
    @Test fun stableConversationsMarkersFilteringAndSearch() = runBlocking {
        ...
        try {
            ...
            assertEquals(2, repository.observeSearch("", "仅标记").first().size)
            assertTrue(repository.observeSearch("%", "全部").first().isEmpty())
            assertTrue(repository.observeSearch("_", "全部").first().isEmpty())
            assertTrue(repository.observeSearch("", "全部").first().isEmpty())
            assertEquals(1, repository.observeSearch("回滚").first().size)
