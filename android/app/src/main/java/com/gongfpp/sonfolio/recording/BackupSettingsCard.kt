...
@Composable internal fun BackupSettingsCard() {
    ...
    Surface(Modifier.fillMaxWidth().padding(top = 13.dp), shape = MaterialTheme.shapes.medium) {
        Column(Modifier.padding(13.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            Text("完整备份与恢复", fontWeight = FontWeight.Bold, fontSize = 16.sp)
            Text("包含原音、转写、标记、缺口、对话和总结；不包含模型、API Key 与个性化设置。备份未加密，保存到你信任的位置，勿公开分享。请先停止录音，并在完成前留在此页面。", fontSize = 12.sp)
            Row {
            ...
            Text("恢复仅支持空数据库，不覆盖或合并已有记录。原音逐个校验 SHA-256，通过后统一写入数据库。单份备份文字清单上限 64 MB。", fontSize = 11.sp)
